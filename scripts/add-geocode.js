...
/**
 * users who cannot be mapped to correct location
 */
const specialCases = {
  // ╮( ˘ ､ ˘ )╭
  antfu: [-30.613889, 83.6625],
  bencodezen: [-77.0366, 38.895]
}
...
async function main() {
  ...
  for (let item of items) {
    if (item.location) {
      if (specialCases[item.login]) {
        item.geoCode = specialCases[item.login]
        continue
      }
